...
Sync threads is not useful in the above example.
...
Solution. Global memory stores full inputs and outputs, while local memory supports work groups/blocks and private memory supports threads. Global memory has the size in order of GB, local memory in order of KB/work group and private in terms of words/WI. Private memory is fastest to access followed by local and then Global memory.
...
Solution. One way would be to use Taylor Series coefficients upto Q terms (where Q will decide the precision). However this would be computationally expensive as computing factorials would be very hard (which could be optimized a little by dynamic programming). Another method would be to have a look up table of particular values and using interpolation methods to find values in between (Many such algorithms exist).

No in most cases. Computer Datatypes have finite precision and cannot represent numbers with large number of decimal points effectively.
...
Vector length is 7500. Threads in one block = 512.
Number of blocks needed  = ciel (7500/512) = 15
Total number of threads in 15 blocks = 15 * 512 = 7680.  
